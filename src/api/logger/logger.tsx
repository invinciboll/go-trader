...
const LoggerContext = createContext<ReturnType<typeof useLoggerState> | null>(
	null,
);
...
function useLoggerState() {
	...
	const append = (level: LogLevel, message: string) =>
		setLogs((prev) => [
			...prev,
			{ id: _id++, timestamp: new Date(), level, message },
		]);
	return {
		logs,
...
export const LoggerProvider = ({ children }: { children: ReactNode }) => (
	<LoggerContext.Provider value={useLoggerState()}>
		{children}
