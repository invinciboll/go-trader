...
const TradeDeviceCard = () => {
	const { currentMachineState, currentTradeStep, currentScreenshotUrl } =
		useTradeMachine();
	...
	const factor =
		devW && devH
			? Math.min(previewMaxWidth / devW, previewMaxHeight / devH)
			: null;
	...
	return (
		<Card
			title={currentMachineState.toLowerCase()}
			extra={
				<div style={{ width: 200 }}>
					<TradeGlobalProgress />{' '}
				</div>
