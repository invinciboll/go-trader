...
import { Button, Card, Flex, Form, InputNumber, Switch, Tag } from 'antd';
...
const TradeSettings: React.FC = () => {
	...
	const [remoteTradeButtonVisible, setRemoteTradeButtonVisible] =
		useState(false);
	...
	const handleCalibrate = async () => {
		...
		initialize(
			remoteTradeButtonVisible,
			dimensions.deviceWidth,
			dimensions.deviceHeight,
		);
		setCalibrationInfo({
			success: true,
			message: `${dimensions.deviceWidth}px / ${dimensions.deviceHeight}px`,
		});
		...
		<Card title="Trade Settings" variant="borderless" style={{ width: 300 }}>
			<Form layout="vertical">
				...
				<Form.Item label="Remote trade button visible">
					<Switch
						disabled={currentMachineState === 'RUNNING'}
						value={remoteTradeButtonVisible}
						onChange={() => {
							setRemoteTradeButtonVisible((prev) => !prev);
							setNeedRecalibrate(true);
						}}
					/>
				</Form.Item>
				<Form.Item label="Calibrate for your device">
					<Flex gap={8} align="center">
						<Button
							...
							icon={
								needRecalibrate && calibrationInfo ? (
						...
						{calibrationInfo && !needRecalibrate && (
							<Tag
								color={calibrationInfo?.success ? 'green' : 'red'}
								variant={'outlined'}
							>
				...
				<Form.Item label="Prepared application">
					<Switch
						disabled={currentMachineState === 'RUNNING'}
						onClick={() =>
							localStorage.getItem('hide-trade-info-modal') === 'true' ||
							userConfirmedPrep
								? {}
								: setTradeInfoModalVisible(true)
						}
						value={userConfirmedPrep}
						onChange={setUserConfirmedPrep}
					/>
				</Form.Item>
			...
			<TradeInfoModal
				open={tradeInfoModalVisible}
				onClose={() => setTradeInfoModalVisible(false)}
			/>
		</Card>
	);
};
